-- Ensure claimed_approval column exists
DO $$
BEGIN
...
-- Drop existing functions first because we are changing the return type
DROP FUNCTION IF EXISTS get_businesses_extra(text,text[],text,text,text,text,integer,integer);
DROP FUNCTION IF EXISTS business_category(text,text[],integer,integer,integer);
DROP FUNCTION IF EXISTS business_location(text,text[],text,text,integer,integer);
DROP FUNCTION IF EXISTS business_subcategories(text,text[],integer,integer,integer);
DROP FUNCTION IF EXISTS search_businesses(text,text[],text,text,double precision,double precision);
DROP FUNCTION IF EXISTS search_businesses2(text,text[],text,text,double precision,double precision);

-- Update get_businesses_extra to return claimed_approval
...
BEGIN
  ...
  SELECT
    ...
    fb.total_photos,
    fb.snippet,
    COALESCE(fb.claimed_approval, false) as claimed_approval
...
-- Update business_category to return claimed_approval
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(b.claimed_approval, false) as claimed_approval
    FROM 
        public.businesses b
    LEFT JOIN 
...
-- Update business_location to return claimed_approval
...
-- Update business_subcategories to return claimed_approval
...
-- Update search_businesses to return claimed_approval
