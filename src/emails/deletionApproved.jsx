...
export const DeletionApprovedEmail = ({ businessName, businessId, userEmail, siteUrl, siteName }) => {
  ...
  <Html>
    <div style={{ maxWidth: 600, margin: '0 auto', padding: 20, border: '1px solid #ddd', borderRadius: 5, fontFamily: 'Arial, sans-serif', lineHeight: 1.6, color: '#333' }}>
      <div style={{ backgroundColor: '#f8f9fa', padding: 10, textAlign: 'center', borderBottom: '1px solid #ddd' }}>
        <h2>Removal Approved on {finalSiteName}</h2>
      </div>
      <div style={{ padding: 20 }}>
        <p>Hello,</p>
        <p>Your request to remove the listing has been approved.</p>
        <p><strong>Business:</strong> {businessName}</p>
        <p><strong>ID:</strong> {businessId}</p>
        <p>If this was a mistake, please contact support.</p>
        <p>
          <a href={`${siteUrl}`} style={{ display: 'inline-block', padding: '10px 20px', backgroundColor: '#007bff', color: '#fff', textDecoration: 'none', borderRadius: 5 }}>
            Visit Website
          </a>
        </p>
      </div>
...
export default DeletionApprovedEmail
